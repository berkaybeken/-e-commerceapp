...
export default function LoginPage() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (!res.ok) { setErr(t('login.invalid')); return; }
      localStorage.setItem('auth', String(Date.now()));
      window.dispatchEvent(new Event('auth-change'));
      router.push(next);
    } catch { setErr(t('login.unexpected')); } finally { setLoading(false); }
  }

  return (
    <section className="max-w-sm">
      <h1 className="text-2xl font-semibold mb-4">{t('login.title')}</h1>
      <form onSubmit={submit} noValidate className="space-y-3 rounded-2xl card-skin p-4">
        <div>
          <label className="text-sm muted">{t('login.email')}</label>
          <input
            ...
            autoComplete="email"
          />
          {emailErr && <p id="login-email-err" className="mt-1 text-xs text-red-600">{emailErr}</p>}
        </div>

        <div>
          <label className="text-sm muted">{t('login.password')}</label>
          <div className="flex items-stretch gap-2">
            <input
              ...
              aria-describedby={passErr ? 'login-pass-err' : undefined}
          ...
          {passErr && <p id="login-pass-err" className="mt-1 text-xs text-red-600">{passErr}</p>}
        </div>

        {err && <p className="text-sm text-red-600">{err}</p>}

        <button type="submit" disabled={loading} className="btn-primary w-full rounded-xl px-4 py-2">
